...
GET /captains/register

**Description:**  
- Create a new captain account and return a JWT token plus the created captain (password omitted).
...
- `fullname.firstname` must be at least 3 characters (returns 400 if invalid).  
- `password` must be at least 6 characters (returns 400 if invalid).    
- `vehicle.color` must be at least 3 characters (returns 400 if invalid).                                                 
- `vehicle.plate` must be at least 3 characters (returns 400 if invalid).                                                 
- `vehicle.capacity` must be at least 1 (returns 400 if invalid).                                                          
- `vehicle.vehicleType` must be one of 'car', 'motorcycle', or 'auto' (returns 400 if invalid).                             
- All fields listed above are required; missing fields will cause an error.                                               

**Responses:**

- 201 Created
  - Returned when captain is successfully created.
  - Response body example:
...
**Notes / Implementation details:**
- Endpoint is mounted under `/captains` in the app, so route is `/captains/register`.
- Passwords are hashed before being saved; the API response does not include the password.
- The endpoint returns a JWT generated by the server on success.
